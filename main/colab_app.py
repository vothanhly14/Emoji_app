# ================================================================
#  DrawSense AI  ·  Violet Dream  ·  Google Colab Version
#  Gradio + TensorFlow + Pillow + NumPy
#  Chạy lần lượt từng CELL trong Colab
# ================================================================
...
# ┌──────────────────────────────────────────────────────────┐
# │  CELL 2 — Upload model + classes.json lên Colab          │
# └──────────────────────────────────────────────────────────┘
# from google.colab import files
# uploaded = files.upload()   # chọn emoji_model.h5 và classes.json

# ┌──────────────────────────────────────────────────────────┐
# │  CELL 3 — Toàn bộ code app (paste & run)                │
# └──────────────────────────────────────────────────────────┘
import os, json
import numpy as np
...
def predict(sketch_input):
    ...
    B2  Grayscale → Invert (chuẩn QuickDraw)
...
with gr.Blocks(
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=11, elem_classes="panel-glass"):
            ...
            try:
                canvas = gr.Sketchpad(
                    label="", type="pil", show_label=False,
                    brush=gr.Brush(default_size=22, colors=["#000000"]),
                    canvas_size=(480, 480),
                )
            except Exception:
                canvas = gr.Image(
            ...
            with gr.Row():
                ...
                btn_clear   = gr.ClearButton(canvas, value="↺  Xóa", variant="secondary", scale=1)
    ...
    btn_clear.click(fn=lambda: _html_empty(), outputs=[out_html])
